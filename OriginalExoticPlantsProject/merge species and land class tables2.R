...
# land_tab<-list.files( paste(out.dir,"Land_Catagories\\",sep=""),"_point_polygon_summary_factor_of_3.csv",full.names=TRUE)
...
#CAPAD_Presence_absence_lat_long_all_apecies_factor_3
...
#################################
  #presnece
  #CAPAD DAtA
sp_table<-read.csv("F:\\Current_Projects\\exotic_plants\\outputs\\all species\\CAPAD_Presence_absence_lat_long_all_apecies_factor_3.csv")
  sp_table$x<-round(sp_table$x,5)
  sp_table$y<-round(sp_table$y,5)

dat<-read.csv("F:\\Current_Projects\\exotic_plants\\outputs\\Land_Catagories\\ CAPAD _point_polygon_summary_factor_of_3.csv")
...
#merge sp_table and polygon data together
    ...
    write.csv(area_max,"F:\\Current_Projects\\exotic_plants\\data\\website\\CAPAD_species_presence_factor_of_3.csv")
...
area_name<-as.vector(CAPAD_Unique$"PA_ID")
locations2b<-list()
for (i in 1:length(area_name)){
  
  sub_locs_a<-subset(locations2,PA_ID==area_name[i])
  ...
  #aggregate to get for each PA_ID area sum
  ag2 <- aggregate(area_km2 ~ PA_ID, data=sub_locs_a, FUN=sum)
  ag4<- as.data.frame(t(as.data.frame(apply(sub_locs_b,2,FUN=sum))))
  #ag4 <- aggregate(locations4[,6:ncol(locations2)], by=list(sub_locs_b$PA_ID), FUN=sum)
  #combine to get percentage 
  locations5<-as.data.frame(t(as.data.frame(apply(ag4,2,function(z)(round(z/ag2[,2]*100))))))
  #locations6<-t(as.data.frame(cbind(as.vector(ag2$PA_ID),locations5)))
  #reasign column name
  locations5$"PA_ID"<-as.vector(ag2$"PA_ID")
...
#recombine for other information
area_prop<-merge(CAPAD_Unique,locations6,by="PA_ID")             
write.csv(area_prop,"F:\\Current_Projects\\exotic_plants\\data\\website\\CAPAD_species_percent_occupied_factor_3.csv")
...
sp_table<-read.csv("F:\\Current_Projects\\exotic_plants\\outputs\\all species\\CAPAD_restricted_Presence_absence_lat_long_all_apecies_factor_3.csv")
  sp_table$x<-round(sp_table$x,5)
  sp_table$y<-round(sp_table$y,5)

dat<-read.csv("F:\\Current_Projects\\exotic_plants\\outputs\\Land_Catagories\\ CAPAD_restrictedt _point_polygon_summary_factor_of_3.csv")
    dat1<-dat[,c("x","y","PA_ID")]
    dat1$x<-round(dat1$x,5)
    dat1$y<-round(dat1$y,5)
    ...
    #recombine for other information
    area_max<-merge(CAPAD_Unique,ag,by="PA_ID")
    write.csv(area_max,"F:\\Current_Projects\\exotic_plants\\data\\website\\CAPAD_restricted_species_presence_factor_3.csv")
#     
#     # % occumpied
#     #set up dataframes
#     CAPAD2<-merge(dat1, locs,by=c("x","y"),all = FALSE,all.x = TRUE)  
...
#recombine for other information
area_prop<-merge(CAPAD_Unique,locations6,by="PA_ID")             
write.csv(area_prop,"F:\\Current_Projects\\exotic_plants\\data\\website\\CAPAD_restricted_species_percent_occupied_factor_3.csv")
...
sp_table<-read.csv("F:\\Current_Projects\\exotic_plants\\outputs\\all species\\ ramsar_Presence_absence_lat_long_all_apecies_factor_3.csv")

dat<-read.csv("F:\\Current_Projects\\exotic_plants\\outputs\\Land_Catagories\\ ramsar _point_polygon_summary_factor_of_3.csv")
...
write.csv(area_max,"F:\\Current_Projects\\exotic_plants\\data\\website\\RAMSAR_species_presence_factor_3.csv")
...
write.csv(area_prop,"F:\\Current_Projects\\exotic_plants\\data\\website\\RAMSAR_species_percent_occupied_factor_3.csv")
